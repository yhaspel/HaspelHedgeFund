...
@Component({
    ...
    <hf-app-shell [crumbs]="[{ label: 'Fund', link: '/fund' }, { label: 'Autopilot' }]">
          ...
          <div class="head-actions">
            ...
            @if (!a.is_enabled) {
              <span class="pill-wrap">
                ...
                >
                  <span class="dot"></span>disabled<span class="pill-q" aria-hidden="true">?</span>
                </button>
                <hf-popover
                  #pop
                  role="tooltip"
      ...
      @if (ap(); as a) {
        ...
        <section class="card">
          ...
          <ul class="checklist">
            @for (c of a.validation.checks; track c) {
          ...
          @if (!a.validation.passed) {
            <p class="muted">
              Run a passing walk-forward backtest (positive OOS Sharpe + max DD within the hard-halt
              limit) for this strategy to unlock the enable toggle.
            </p>
          }
        </section>
        ...
        <section class="card controls">
          ...
          @if (!a.is_enabled && !a.validation.passed) {
            <a
          ...
          @if (!a.is_enabled) {
            <button
              ...
              [title]="
                a.validation.passed
                  ? ''
                  : 'Validation gate not passed — run a validation backtest first'
              "
              (click)="enable()"
            >
              Enable autopilot
          ...
          @if (a.is_enabled) {
            <button class="btn" (click)="disable()" [disabled]="busy()">Disable</button>
          }
          @if (a.state === 'halted') {
            <button class="btn" (click)="resume()" [disabled]="busy()">Resume (un-halt)</button>
          }
          @if (a.is_enabled) {
          ...
          <div class="sched-head">
            ...
            <div class="seg" role="group" aria-label="Schedule editor mode">
              ...
              <button
                type="button"
                class="opt"
                [class.on]="scheduleMode === 'advanced'"
          ...
          @if (scheduleMode === 'simple') {
            <div>
              <div class="sched-row">
                ...
                <label class="fld"
                  >Time <small>({{ a.timezone }})</small>
                  <input
                    class="input"
                    type="time"
                    [(ngModel)]="schedTime"
                    (ngModelChange)="syncCron()"
                  />
                </label>
                ...
                <div class="days" role="group" aria-label="Days of week">
                  @for (d of weekdays; track d) {
                    <button
                      type="button"
                      class="daychip"
                      [class.on]="schedDays.includes(d.dow)"
                      [attr.aria-pressed]="schedDays.includes(d.dow)"
          ...
          @if (scheduleMode === 'advanced') {
            <div class="sched-adv">
              ...
              @if (a.cron_description) {
                <small class="hint">{{ a.cron_description }} · {{ a.timezone }}</small>
              }
              <small class="hint"
                >minute hour day-of-month month day-of-week — e.g. <code>30 16 * * 5</code> = Fri
                16:30.</small
              >
            </div>
          }

          <button class="btn" (click)="save()" [disabled]="busy() || !scheduleValid()">
            Save changes
          </button>
        </section>
        ...
        <section class="card">
          ...
          <div class="grid">
            <label
              ...
              <small class="hint"
                >Governs autonomous runs for this account only — independent of your
                <a routerLink="/settings/models">Settings → Models</a> defaults (those apply to
                manual runs &amp; the questionnaire).</small
              >
            </label>
            <label
              >Target vol % <input class="input" type="number" [(ngModel)]="form.target_vol_pct"
            /></label>
            <label
              >Soft cut % <input class="input" type="number" [(ngModel)]="form.dd_soft_cut_pct"
            /></label>
            <label
              >Hard halt % <input class="input" type="number" [(ngModel)]="form.dd_hard_halt_pct"
            /></label>
            <label
              >Max orders/day
              <input class="input" type="number" [(ngModel)]="form.max_orders_per_day"
            /></label>
            <label
              >Liquidity ADV cap %
              <input class="input" type="number" [(ngModel)]="form.liquidity_adv_cap_pct"
            /></label>
            <label class="chk"
              ><input type="checkbox" [(ngModel)]="form.flatten_on_halt" /> Flatten to cash on hard
              halt</label
            >
          </div>
          ...
          @if (councilModels().length) {
            <div class="council">
              <div class="council-hd">
                Models this account's runs use · preset <b>{{ form.model_preset }}</b>
              </div>
              <ul class="council-list">
                @for (m of councilModels(); track m) {
                  <li>
                    <span class="cm-roles">{{ m.roles.join(' · ') }}</span>
                    <span class="cm-arrow">→</span>
                    <span class="cm-name">{{ m.name }}</span>
                    @if (m.tier) {
                      <span class="cm-tier">· {{ m.tier }}</span>
                    }
                  </li>
                }
              </ul>
              <p class="council-note">
                ...
                diversity — this is what the <b>{{ form.model_preset }}</b> preset resolves to.
                Autonomous runs use this preset only, independent of your
      ...
      <section class="card">
        ...
        @if (runs().length === 50) {
          <p class="muted">Showing the 50 most recent fires.</p>
        }
        <a class="text-[var(--acc-info-fg)]" [routerLink]="['/strategies', strategyId]"
          >Open strategy cycles &amp; runs →</a
        >
      </section>
  ...
  styles: [
    `
      ...
      .council-list .cm-roles {
        color: var(--text-3);
        min-width: 130px;
      }
      .council-list .cm-arrow {
        color: var(--text-3);
      }
      .council-list .cm-name {
        font-family: var(--font-mono);
        color: var(--text);
      }
      .council-list .cm-tier {
        color: var(--text-3);
        font-size: 11px;
      }
      .council-note {
...
export class AutopilotPanelPage implements OnInit {
  ...
  private readonly ROLE_GROUP: Record<string, string> = {
    ...
    graham: 'Personas',
    wood: 'Personas',
    druckenmiller: 'Personas',
    burry: 'Personas',
    damodaran: 'Personas',
    lynch: 'Personas',
    fundamentals: 'Analytical',
    technicals: 'Analytical',
    valuation: 'Analytical',
    sentiment: 'Analytical',
    macro: 'Macro',
    news_digest: 'News',
    risk_manager: 'Risk / PM / CIO',
    portfolio_manager: 'Risk / PM / CIO',
    cio: 'Risk / PM / CIO',
  };
  ...
  onPresetChange(): void {
    this.refreshCouncil(this.form.model_preset ?? '');
  }
  ...
  save(): void {
    this.run(this.store.saveAutopilot(this.strategyId, this.form), 'Saved.');
  }
  ...
  schedulePreview(): string {
    ...
    if (this.schedFreq === 'monthly')
      return `Runs monthly on day ${this.schedDom || 1} at ${t} (${tz}).`;
    const names = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    const sel = [1, 2, 3, 4, 5, 6, 0]
      .filter((d) => this.schedDays.includes(d))
      .map((d) => names[d]);
    return sel.length
      ? `Runs weekly on ${sel.join(', ')} at ${t} (${tz}).`
      : 'Pick at least one day to run.';
  }
