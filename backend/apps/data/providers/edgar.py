...
class EdgarProvider:
    ...
    # A cached filing set is only trusted while its NEWEST row is this recent
    # relative to ``as_of``. 10-Qs land ~quarterly, so ~100 days is one filing
    # cycle plus slack.
    MAX_FILING_GAP_DAYS = 100
    ...
    def get_recent_filings(
        ...
        *,
        ...
        # Having ``limit`` rows is not the same as having the LATEST ``limit``
        # rows: once the cache filled up, EDGAR was never consulted again and
        # the personas kept reading years-old 10-K/10-Qs. Only short-circuit
        # while the newest cached filing is recent relative to `as_of` — a
        # 10-Q lands each quarter, so ~100 days is one filing cycle plus slack.
        if len(cached) >= limit and (as_of - cached[0].filed_at).days <= self.MAX_FILING_GAP_DAYS:
